...
export class CatalogService {
  ...
  async readBundledAll(): Promise<CatalogSnapshot | undefined> {
    ...
    try {
      ...
      const parsed = JSON.parse(raw) as { manifest?: { generatedAt?: string; totalHits?: number }; entries?: CatalogEntry[] };
  ...
  async fetchPrebuiltAll(remoteUrl: string, signal?: AbortSignal): Promise<CatalogSnapshot> {
    ...
    const parsed = JSON.parse(await response.text()) as { manifest?: { generatedAt?: string; totalHits?: number }; entries?: CatalogEntry[] };
  ...
  async fetchAll(signal?: AbortSignal): Promise<CatalogSnapshot> {
    const { entries, totalHits } = await fetchGitHubTopic('dsh-plugin', signal);
    if (entries.length === 0) throw new Error('github topic search returned no repos');
   ...
   */
  async fetchAllFull(signal?: AbortSignal, opts?: { paceMs?: number; maxQueries?: number }): Promise<CatalogSnapshot> {
