...
import { updatePlugin, type LifecycleContext } from './lifecycle.ts';
import { readProfileManifest } from './installed.ts';
...
export function compareVersions(a: string, b: string): number {
  ...
  for (let i = 0; i < len; i += 1) {
    ...
    if (aNum === -1 && xa !== xb) return xa < xb ? -1 : 1;
  }
  return 0;
}
...
let latestCache: LatestCache | undefined;

async function cachedLatest(signal?: AbortSignal): Promise<{ latest?: string; error?: string }> {
  const now = Date.now();
  if (latestCache) {
    const ttl = latestCache.latest !== undefined ? LATEST_OK_TTL_MS : LATEST_ERR_TTL_MS;
    if (now - latestCache.at < ttl) return { latest: latestCache.latest, error: latestCache.error };
  }
  try {
...
export async function selfSpec(ctx: LifecycleContext): Promise<string> {
...
export async function selfStatus(ctx: LifecycleContext, signal?: AbortSignal): Promise<SelfStatus> {
 ...
 */
export async function selfUpdate(ctx: LifecycleContext): Promise<LifecycleResult & { restartRequired?: boolean }> {
  const spec = await selfSpec(ctx);
  const result = await updatePlugin(ctx, PANEL_PACKAGE, spec);
  if (result.ok) latestCache = undefined; // next check re-reads upstream
