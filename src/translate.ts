...
export interface LlmLike {
  stream(options: {
    ...
    model: string;
    system?: string;
    messages: Array<{ id?: string; role: string; content: Array<{ type: string; text: string }>; source?: { kind: string } }>;
    maxTokens?: number;
    temperature?: number;
...
export interface ModelRouteLike { provider?: string; model?: string }
...
type CachedTranslation = TranslationResult & { sourceHash: string };

function sourceHash(item: TranslationItem): string {
...
function acceptTranslation(item: TranslationItem, titleZh: unknown, descriptionZh: unknown): TranslationResult | undefined {
  const title = cleanText(titleZh) || item.title;
  const description = cleanText(descriptionZh);
  if (!description && item.description.trim()) return undefined;
  // An English description echoed verbatim is an incomplete translation.
  if (!/[\u3400-\u9fff]/u.test(item.description) && item.description.trim() === description) return undefined;
  const latinWords = item.description.match(/[A-Za-z]{2,}/g)?.length ?? 0;
  if (latinWords >= 3 && !/[\u3400-\u9fff]/u.test(description)) return undefined;
...
export function parseTranslationReply(reply: string): Map<number, [string, string]> {
  const result = new Map<number, [string, string]>();
  const parseObject = (value: unknown): void => {
    if (!value || typeof value !== 'object') return;
    const record = value as Record<string, unknown>;
    for (const key of ['translations', 'results', 'items']) {
      if (Array.isArray(record[key])) (record[key] as unknown[]).forEach(parseObject);
    }
    const rawIndex = typeof record.index === 'number' ? record.index : Number(record.index);
    if (!Number.isInteger(rawIndex) || rawIndex < 1) return;
    ...
    if (title || description) result.set(rawIndex - 1, [title, description]);
  };

  const unfenced = reply.replace(/```(?:json)?\s*|```/gi, '').trim();
  try {
    const json = JSON.parse(unfenced) as unknown;
    if (Array.isArray(json)) json.forEach(parseObject);
    else parseObject(json);
  } catch {
    for (const rawLine of unfenced.split(/\r?\n/)) {
      const line = rawLine.trim();
      if (!line) continue;
      if (line.startsWith('{')) {
        try { parseObject(JSON.parse(line)); } catch { /* try legacy syntax below */ }
      }
...
export function chunkText(chunk: unknown): string {
  ...
  if (chunk !== null && typeof chunk === 'object') {
    ...
    if (record.delta && typeof record.delta === 'object') return chunkText(record.delta);
    if (typeof record.content === 'string') return record.content;
    if (Array.isArray(record.content)) return record.content.map(chunkText).join('');
    if (record.message && typeof record.message === 'object') return chunkText(record.message);
    if (record.part && typeof record.part === 'object') return chunkText(record.part);
  }
  return '';
}
...
export class TranslationStore {
  ...
  private cache: Record<string, CachedTranslation> = {};
  ...
  async load(): Promise<void> {
    try {
      const parsed = JSON.parse(await readFile(this.file, 'utf8')) as Record<string, CachedTranslation>;
      this.cache = parsed ?? {};
    } catch {
      this.cache = {};
    }
  ...
  async setMany(items: TranslationItem[], results: TranslationResult[]): Promise<void> {
    const sources = new Map(items.map((item) => [item.id, item]));
    for (const result of results) {
      const source = sources.get(result.id);
      if (source) this.cache[result.id] = { ...result, sourceHash: sourceHash(source) };
    }
  ...
  partition(items: TranslationItem[]): { missing: TranslationItem[]; cached: TranslationResult[] } {
...
function promptFor(items: TranslationItem[]): string {
  return items.map((item, index) => JSON.stringify({ index: index + 1, title: item.title, description: item.description })).join('\n');
}

async function translateWithLlm(route: TranslationRoute, items: TranslationItem[], signal?: AbortSignal): Promise<TranslationResult[]> {
  if (!route.llm || !route.route?.provider || !route.route?.model) return [];
  let reply = '';
...
async function googleFallback(route: TranslationRoute, items: TranslationItem[], signal?: AbortSignal): Promise<TranslationResult[]> {
  ...
  for (let offset = 0; offset < items.length; offset += 6) {
    ...
    const settled = await Promise.all(batch.map(async (item): Promise<TranslationResult | undefined> => {
      if (!item.description.trim()) return { id: item.id, titleZh: item.title, descriptionZh: item.description };
      try {
        ...
        const description = payload[0].map((part: unknown) => Array.isArray(part) && typeof part[0] === 'string' ? part[0] : '').join('');
        return acceptTranslation(item, item.title, description);
      } catch {
        return undefined;
      }
    }));
    translated.push(...settled.filter((value): value is TranslationResult => value !== undefined));
  }
  return translated;
}
...
export async function translateBatch(
  ...
  if (fresh.length > 0) await store.setMany(missing, fresh);
