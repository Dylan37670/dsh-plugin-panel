...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var css = [
      ...
      ".pp-ops h3{margin:0 0 4px;font-size:11px;color:var(--dsw-alias-label-tertiary,#8a919b)}",
    ...
    var zh = {
      ...
      "src.allTip": "GitHub 上所有打了 dsh-plugin 标签的仓库（3000+，含实验性与未完成项目）。GitHub 搜索 API 单次查询上限 1000 条，默认先显示按星标排序的前 1000 条，可点「获取全部」拉取完整列表（约 1-5 分钟）。",
      "src.fetchAll": "获取全部",
      ...
      "settings.remoteUrl": "远程目录 URL（留空 = awesome 列表）",
    ...
    var en = {
      ...
      "settings.remoteUrl": "Remote catalog URL (empty = awesome list)",
    ...
    function EntryCard(props) {
      ...
      if (installed) {
        ...
        actions.push({ key: "install", label: t("entry.install"), primary: true, action: "install" });
      }
      ...
      return el("div", { className: "pp-card", ref: cardRef },
        ...
        el("div", { className: "pp-actions" },
          ...
          installed ? el("span", { className: "pp-installed" }, t("entry.installed")) : null
        )
      );
    }
    ...
    function PluginPanelApp(props) {
      ...
      var pushOp = useCallback(function (label) {
        ...
        setOps(function (prev) { return [op].concat(prev).slice(0, 30); });
        return op.id;
      }, []);
      ...
      var finishOp = useCallback(function (id, state, detail) {
        setOps(function (prev) {
          return prev.map(function (op) {
            ...
            return { ...op, state: state, detail: detail || op.detail };
          });
        });
      }, []);
      ...
      var loadAll = useCallback(function (lensArg) {
        ...
        api("/catalog?lens=" + encodeURIComponent(l)).then(function (res) {
          if (res.ok) setCatalog({ entries: res.entries, source: res.source, fetchedAt: res.fetchedAt, lens: res.lens, totalHits: res.totalHits });
      ...
      var loadAll = useCallback(function (lensArg) {
        ...
        api("/catalog?lens=" + encodeURIComponent(l)).then(function (res) {
          if (res.ok) {
            setCatalog({ entries: res.entries, source: res.source, fetchedAt: res.fetchedAt, lens: res.lens, totalHits: res.totalHits });
      ...
      function refreshLens(l, full) {
        var opId = pushOp(full ? t("src.fetchAll") : t(l === "all" ? "src.fetchingAll" : "src.fetchingCurated"));
        setFetching(l);
        api("/refresh", "POST", { remoteCatalogUrl: urlDraft, lens: l, full: !!full }).then(function (res) {
          if (res.ok) {
            setCatalog({ entries: res.entries, source: res.source, fetchedAt: res.fetchedAt, lens: res.lens, totalHits: res.totalHits });
      ...
      function setLens(next) {
        ...
        api("/state", "POST", {
          state: { ...panelState, settings: { ...panelState.settings, catalogSource: next } }
        }).catch(function () {});
      ...
      function entryInstalled(e) {
        ...
        if (installedNames[e.install]) return true;
        if (e.npm && installedNames[e.npm]) return true;
        if (e.repo) {
          var base = e.repo.split("#")[0].replace(/\/$/, "").split("/").pop();
          for (var key in installedNames) {
            if (key.split("/").pop() === base) return true;
          }
        }
        return false;
      }
      ...
      function setSort(next) {
        ...
        api("/state", "POST", {
          state: { ...panelState, settings: { ...panelState.settings, sort: next } }
        }).catch(function () {});
      }

      function refresh() {
        refreshLens(lens, lens === "all");
      }
      ...
      function persistEmbeddingConfig(enabled) {
        var next = { ...panelState, settings: { ...panelState.settings, embedding: embeddingDraft(enabled) } };
        return api("/state", "POST", { state: next }).then(function (res) {
      ...
      // v6.1: auto-save the embedding config as the user types (key/model/URL),
      // so the status reflects the key immediately without an explicit save.
      var embSaveTimer = useRef(null);
      useEffect(function () {
        var emb = panelState.settings && panelState.settings.embedding;
        var savedKey = emb ? emb.apiKey || "" : "";
        var savedModel = emb ? emb.model || "" : "";
        var savedUrl = emb ? emb.baseUrl || "" : "";
        if (embKeyDraft.trim() === savedKey && (embModelDraft.trim() || "BAAI/bge-m3") === savedModel && embUrlDraft.trim() === savedUrl) return;
        if (embSaveTimer.current) clearTimeout(embSaveTimer.current);
        embSaveTimer.current = setTimeout(function () {
          persistEmbeddingConfig(semanticOn).then(refreshEmbStatus).catch(function () {});
        }, 600);
      }, [embKeyDraft, embModelDraft, embUrlDraft, panelState.settings]);
      ...
      function toggleSemantic(next) {
        setSemanticOn(next);
        var nextSettings = { ...panelState.settings, embedding: { enabled: next, provider: "硅基流动", baseUrl: embUrlDraft.trim(), model: embModelDraft.trim() || "BAAI/bge-m3", apiKey: embKeyDraft.trim() } };
        api("/state", "POST", { state: { ...panelState, settings: nextSettings } }).then(function (res) {
          if (res.ok) {
            setPanelState(res.state);
            refreshEmbStatus();
          }
      ...
      function buildVectorIndex() {
        if (buildingIndex) return; // v6.4: never start a second build
        var opId = pushOp(t("sem.build"));
        setBuildingIndex(true);
        setSemanticError("");
        persistEmbeddingConfig(semanticOn).then(function () {
          return api("/embedding/build", "POST", {});
        }).then(function (res) {
          finishOp(opId, res.ok ? "ok" : "error", res.ok ? (t("sem.status", { model: res.model, count: String(res.count) })) : res.message);
      ...
      useEffect(function () {
        ...
        semTimer.current = setTimeout(function () {
          ...
          setSemanticError("");
          persistEmbeddingConfig(true).then(function () {
            return api("/search-semantic", "POST", { query: q, limit: 80 });
          }).then(function (res) {
      ...
      function runLifecycle(entry, action) {
        ...
        var body = action === "install" ? { spec: spec, label: entry.title } :
                   action === "update" ? { name: spec, spec: spec } :
                   { name: spec };
        api("/" + action, "POST", body).then(function (res) {
      ...
      function saveSettings() {
        var next = { ...panelState, settings: { ...panelState.settings, remoteCatalogUrl: urlDraft } };
        api("/state", "POST", { state: next }).then(function (res) {
          if (res.ok) {
            setPanelState(res.state);
            setSavedFlash(true);
            setTimeout(function () { if (mounted.current) setSavedFlash(false); }, 1500);
      ...
      var refreshBusy = ops.some(function (op) { return op.label === t("panel.refresh") && op.state === "running"; });
          ...
          el("aside", { className: "pp-drawer", role: "dialog", "aria-label": t("panel.title") },
            el("div", { className: "pp-header" },
              ...
              el("span", { className: "pp-cached" },
                catalog.fetchedAt
                  ? (catalog.lens === "all" ? t("panel.indexBuiltAt", { time: fmtTime(catalog.fetchedAt) }) : t("panel.cachedAt", { time: fmtTime(catalog.fetchedAt) }))
                  : t("panel.noCache")
              ),
            ...
            semanticOn
              ? el("div", { className: "pp-sembar" },
                  !keyConfigured
                    ...
                    : (embStatus.stale
                        ? el("button", { type: "button", className: "pp-sembtn", onClick: buildVectorIndex }, t("sem.stale", { reason: embStatus.staleReason || t("sem.notBuilt") }))
                        : (embStatus.built
                        ? el("span", null, t("sem.status", { model: embStatus.model || embCfg.model, count: String(embStatus.count) }))
                        : el("button", { type: "button", className: "pp-sembtn", "data-busy": buildingIndex || undefined, disabled: buildingIndex, onClick: buildVectorIndex }, buildingIndex ? t("sem.building") : t("sem.build")))),
            ...
            el("div", { className: "pp-srcrow" },
              ...
              lens === "all" && fetching !== "all" && catalog.entries.length < (catalog.totalHits || 0)
                ? el("button", { type: "button", className: "pp-srcbtn", onClick: function () { refreshLens("all", true); } }, t("src.fetchAll"))
                : null
            ),
