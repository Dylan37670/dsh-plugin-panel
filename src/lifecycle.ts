...
import { readManualRegistrations } from './installed.ts';
...
function runCommand(file: string, args: string[], cwd: string, timeoutMs: number): Promise<SpawnResult> {
...
function winCmd(args: string[]): { file: string; args: string[] } {
  return {
...
export function runNode(script: string, args: string[], cwd: string, timeoutMs = 300_000): Promise<SpawnResult> {
...
export function findDshBin(): string | undefined {
  ...
  if (argvBin && /[\\/]bin\.js$/.test(argvBin) && existsSync(argvBin)) return argvBin;
...
export async function checkPnpm(): Promise<boolean> {
  try {
...
export async function setupPnpm(): Promise<LifecycleResult> {
...
export function updateTarget(name: string, spec?: string): string {
  ...
  if (/^(?:link|file|workspace|portal|patch):/i.test(value) || /^[A-Za-z]:[\\/]/.test(value) || value.startsWith('./') || value.startsWith('../')) return value;
...
export async function restoreBackup(ctx: LifecycleContext, backup: BackupRef): Promise<void> {
  const profileDir = join(ctx.dshHome, 'profiles', ctx.profile);
  ...
  for (const name of backup.missing) await rm(join(profileDir, name), { force: true }).catch(() => undefined);
}

function pnpmFailureHint(output: string): string | undefined {
  ...
  if (output.includes('ERR_PNPM_FETCH_404')) return '某个 profile 依赖在 npm 中不存在或需要登录；pnpm 会因此阻止所有安装，请检查 profile/package.json 中是否有上次失败留下的依赖。';
  if (output.includes('ERR_PNPM_PUBLIC_HOIST_PATTERN_DIFF')) return 'profile 的 node_modules 由不同版本的 pnpm 创建；自动重建后重试仍失败。';
  if (output.includes('ERR_PNPM_ADDING_TO_ROOT')) return '当前 pnpm 拒绝修改 workspace 根目录，请升级 pnpm 后重试。';
  return undefined;
}

/** Only retry failures that are plausibly transient transport failures. */
export function isTransientUpdateFailure(output: string): boolean {
  return /\b(?:ETIMEDOUT|ECONNRESET|ECONNREFUSED|EAI_AGAIN|ENOTFOUND)\b|fetch failed|ERR_PNPM_META_FETCH_FAIL|ERR_PNPM_FETCH_5\d\d|\b(?:502|503|504)\b/i.test(output);
}

/** A useful but deliberately non-sensitive summary for the operation log. */
export function updateFailureHint(output: string, retried = false): string {
  const pnpm = pnpmFailureHint(output);
  if (pnpm) return pnpm;
  if (isTransientUpdateFailure(output)) {
    return retried
      ? '网络或 GitHub 拉取失败，自动重试一次后仍未成功。'
      : '网络或 GitHub 拉取暂时失败。';
  }
  if (/ERR_PNPM_/i.test(output)) return 'pnpm 无法完成依赖更新；已回滚，可在终端重试后查看详细错误。';
  return '更新命令失败；已回滚。';
}

/** Run `dsh plugin --profile <p> <args...>`. */
export async function runDshPlugin(ctx: LifecycleContext, args: string[], timeoutMs = 900_000): Promise<SpawnResult> {
  const dshBin = ctx.dshBin ?? findDshBin();
  if (!dshBin) throw new Error('找不到 dsh CLI（未设置 DSH_BIN，也不在运行进程中）');
  const node = process.execPath;
  const profileDir = join(ctx.dshHome, 'profiles', ctx.profile);
  const invoke = (next: string[]) => runNode(dshBin, ['plugin', '--profile', ctx.profile, ...next], profileDir, timeoutMs);
  ...
  if (result.code !== 0 && result.output.includes('ERR_PNPM_PUBLIC_HOIST_PATTERN_DIFF')) {
    ...
    if (repaired.code === 0) result = await invoke(args);
  }
  if (result.code !== 0
...
function panelManualBlock(packageName: string): string {
...
async function appendPanelManual(ctx: LifecycleContext, packageName: string): Promise<void> {
...
async function removePanelManual(ctx: LifecycleContext, packageName: string): Promise<boolean> {
  const file = join(ctx.dshHome, 'profiles', ctx.profile, 'cordis.patch.yml');
  const current = await readOptional(file);
  const block = panelManualBlock(packageName);
...
export async function verifyHostPlugin(ctx: LifecycleContext, packageName: string): Promise<{ ok: boolean; reason?: string }> {
  const packageDir = join(ctx.dshHome, 'profiles', ctx.profile, 'node_modules', packageName);
  let manifest: PluginManifest;
  try { manifest = JSON.parse(await readFile(join(packageDir, 'package.json'), 'utf8')) as PluginManifest; }
  catch { return { ok: false, reason: '找不到已安装包的 package.json' }; }
  if (manifest.name && manifest.name !== packageName) return { ok: false, reason: `真实包名不匹配：${manifest.name}` };
  let entry = manifest.main;
  const rootExport = typeof manifest.exports === 'string' ? manifest.exports : manifest.exports?.['.'];
  if (!entry && typeof rootExport === 'string') entry = rootExport;
  if (!entry && rootExport && typeof rootExport === 'object') entry = Object.values(rootExport).find((value): value is string => typeof value === 'string');
  entry ??= 'index.js';
  const entryFile = join(packageDir, entry);
  if (!existsSync(entryFile)) return { ok: false, reason: `插件入口不存在：${entry}` };
  try {
    const mod = await import(`${pathToFileURL(entryFile).href}?plugin-panel=${Date.now()}`) as { apply?: unknown; default?: unknown };
    const fallback = mod.default as { apply?: unknown } | undefined;
    if (typeof mod.apply !== 'function' && typeof fallback?.apply !== 'function' && typeof mod.default !== 'function') return { ok: false, reason: '包没有可加载的 Cordis apply 入口' };
    return { ok: true };
  } catch (error) {
...
export async function installPlugin(ctx: LifecycleContext, spec: string, label: string): Promise<LifecycleResult> {
  ...
  try {
    ...
    let activation: 'bundle' | 'manual' | undefined;
    let verification = packageName ? await verifyBundle(ctx, packageName) : { ok: false, reason: '无法确认真实包名' };
    if (result.code === 0 && packageName && verification.ok) activation = 'bundle';
    if (result.code === 0 && packageName && !verification.ok) {
      ...
      if (host.ok) {
        ...
        if (verification.ok) activation = 'manual';
      } else {
        verification = host;
      }
    }
...
export async function updatePlugin(ctx: LifecycleContext, name: string, spec: string | undefined, manual = false): Promise<LifecycleResult> {
  const backup = await backupProfile(ctx);
  ...
  try {
    let result = await runDshPlugin(ctx, ['add', target]);
    // A failed DNS/HTTP connection is common enough on GitHub specs to make
    // one immediate retry worthwhile.  Never retry policy, validation or
    // package errors: those need an explicit user action instead.
    const retried = result.code !== 0 && isTransientUpdateFailure(result.output);
    if (retried) result = await runDshPlugin(ctx, ['add', target]);
    ...
    if (result.code === 0 && active) {
      ...
      return {
        ...
        message: changed
          ? `已更新 ${name}（需重启 GUI 生效${retried ? '；网络重试后成功' : ''}）。`
          : `${name} 已经是最新版。`,
      };
    }
    await restoreBackup(ctx, backup);
    return {
      ...
      packageName: name,
      message: result.code === 0
        ? `更新后校验失败：${verification.reason ?? '未知原因'}；已回滚。`
        : `${updateFailureHint(result.output, retried)} 已回滚到备份（${backup.path}）。`,
    };
  } catch (error) {
    await restoreBackup(ctx, backup).catch(() => undefined);
    return {
...
export async function uninstallPlugin(
  ...
  name: string,
  options: { manual?: boolean; panelManaged?: boolean } = {},
  ...
  try {
    ...
    if (result.code === 0 && options.manual) await removePanelManual(ctx, name);
    const after = await readProfile(ctx);
