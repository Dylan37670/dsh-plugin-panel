...
export async function runDshPlugin(ctx: LifecycleContext, args: string[], timeoutMs = 120_000): Promise<SpawnResult> {
...
export async function uninstallPlugin(ctx: LifecycleContext, name: string): Promise<LifecycleResult> {
  const backup = await backupProfile(ctx);
