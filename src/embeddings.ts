...
export function cosine(a: number[], b: number[]): number {
  if (a.length !== b.length) throw new RangeError(`向量维度不一致：${a.length} != ${b.length}`);
  let dot = 0;
  let na = 0;
  let nb = 0;
  const len = a.length;
...
export function embeddingEndpoint(baseUrl: string): string {
  ...
  if (!base) throw new Error('未配置嵌入 API 地址');
  return /\/embeddings$/i.test(base) ? base : `${base}/embeddings`;
}

/** Call the provider to embed a batch of texts (≤100 per call). */
export async function embedTexts(config: EmbeddingConfig, texts: string[], signal?: AbortSignal): Promise<number[][]> {
  ...
  for (const [position, item] of data.entries()) {
    ...
    if (item.embedding && index >= 0 && index < result.length) result[index] = item.embedding;
  }
  for (let i = 0; i < result.length; i += 1) {
    if (!result[i]) throw new Error(`嵌入响应缺少第 ${i} 条（模型 ${config.model}）`);
...
export function catalogHash(entries: CatalogEntry[]): string {
  ...
  for (const entry of entries) hash.update(entry.id).update('\0').update(entrySearchText(entry)).update('\0');
  return hash.digest('hex');
}

/** Explain why an index cannot safely serve the current configuration. */
export function indexCompatibilityIssue(
  index: VectorIndex,
  config: EmbeddingConfig,
  entries?: CatalogEntry[],
): string | undefined {
  if (index.version !== 2) return '索引格式已升级';
  if (index.model !== config.model) return `模型已从 ${index.model} 改为 ${config.model}`;
  if (index.endpoint !== embeddingEndpoint(config.baseUrl)) return '嵌入 API 地址已更改';
  if (index.dim <= 0) return '索引维度无效';
  if (index.vectors.some((record) => record.v.length !== index.dim)) return '索引中存在维度不一致的向量';
  if (entries && index.catalogHash !== catalogHash(entries)) return '插件目录已变化';
  return undefined;
}
...
export class VectorStore {
  ...
  async load(): Promise<VectorIndex | undefined> {
    if (this.index) return this.index;
    try {
      const parsed = JSON.parse(await readFile(this.file, 'utf8')) as VectorIndex;
...
export async function buildIndex(
  ...
  for (let i = 0; i < entries.length; i += batchSize) {
    if (signal?.aborted) throw new Error('索引构建已取消');
...
export async function semanticRank(
  ...
  if (!index || index.vectors.length === 0) throw new Error('向量索引未构建');
  const incompatibility = indexCompatibilityIssue(index, config, entries);
  if (incompatibility) throw new Error(`向量索引已过期（${incompatibility}），请重新构建`);
