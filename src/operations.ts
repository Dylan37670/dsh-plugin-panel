...
function normalize(value: unknown): OperationView | undefined {
  if (!value || typeof value !== 'object') return undefined;
  const row = value as Partial<OperationView>;
  if (typeof row.id !== 'string' || typeof row.startedAt !== 'number') return undefined;
...
export class OperationStore {
  ...
  async load(): Promise<OperationView[]> {
    if (this.loaded) return this.get();
    if (!this.loading) this.loading = (async () => {
      try {
        const parsed = JSON.parse(await readFile(this.file, 'utf8')) as unknown[];
        const now = Date.now();
        this.current = (Array.isArray(parsed) ? parsed : []).map(normalize).filter((x): x is OperationView => !!x)
          .map((op) => op.state === 'running' ? { ...op, state: 'error' as const, detail: '上次运行被中断', finishedAt: now } : op)
  ...
  get(): OperationView[] { return structuredClone(this.current); }

  async upsert(value: unknown): Promise<OperationView[]> {
    await this.load();
