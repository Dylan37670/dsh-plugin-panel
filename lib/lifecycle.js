...
export function isTransientUpdateFailure(output) {
...
export function updateFailureHint(output, retried = false) {
    ...
    if (/ERR_PNPM_/i.test(output))
        return 'pnpm 无法完成依赖更新；已回滚，可在终端重试后查看详细错误。';
    return '更新命令失败；已回滚。';
}
...
export async function updatePlugin(ctx, name, spec, manual = false) {
    ...
    try {
        ...
        if (retried)
            result = await runDshPlugin(ctx, ['add', target]);
