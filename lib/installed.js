...
export async function readManualRegistrations(dshHome, profile) {
    const registrations = new Map();
    let text = '';
    try {
        text = await readFile(join(dshHome, 'profiles', profile, 'cordis.patch.yml'), 'utf8');
    }
    catch {
...
export async function readProfileManifest(dshHome, profile) {
    try {
        return JSON.parse(await readFile(join(dshHome, 'profiles', profile, 'package.json'), 'utf8'));
    }
    catch {
        return {};
    }
...
export async function readProfileBundles(dshHome, profile) {
...
export async function detectInstalled(dshHome, profiles) {
    const items = [];
    const seen = new Set();
