...
function categoryFromSection(section) {
    ...
    if (s.includes('skill'))
        return 'skill';
    if (s === 'ui' || s.includes('theme') || s.includes('appearance') || s.includes('ui enhance'))
        return 'client';
...
export function installSpecFromCommand(command) {
    if (!command)
        return undefined;
...
export function parseCommunityRegistry(file) {
    const entries = [];
    for (const plugin of file.plugins ?? []) {
        if (!plugin.url?.startsWith('https://github.com/') || !plugin.name)
            continue;
        ...
        entries.push({
            ...
            ...(install ? { install, installVerified: true, installSource: 'community' } : {}),
 ...
 */
export async function fetchCommunityRegistry(url = DEFAULT_INSTALL_REGISTRY_URL, options = {}) {
...
export class CatalogService {
    ...
    async readBundledCurated() {
        if (this.bundledCurated)
            return this.bundledCurated;
        try {
    ...
    async readCache(lens) {
        ...
        try {
            ...
            if (Array.isArray(parsed.entries)) {
                ...
                if (lens === 'all' && !parsed.entries.some((entry) => entry.installVerified === true))
                    return undefined;
    ...
    async snapshot(lens = 'curated') {
        ...
        if (bundled)
            return { ...bundled, entries: await this.enrichStars(bundled.entries) };
        if (cache)
            return { ...cache, entries: await this.enrichStars(cache.entries) };
     ...
     */
    async cacheBundledCurated() {
        const bundled = await this.readBundledCurated();
        if (!bundled)
            return;
        const snapshot = { ...bundled, source: 'cache', partial: false };
    ...
    async fetchCurated(remoteUrl, signal) {
        ...
        if (configuredUrl === '') {
            ...
            if (!response.ok)
                throw new Error(`catalog fetch failed: HTTP ${response.status} from ${configuredUrl}`);
    ...
    async fetchPrebuiltAll(remoteUrl, signal) {
        ...
        const headers = { 'User-Agent': 'dsh-plugin-panel' };
        if (previous?.etag)
        ...
        if (parsed.manifest?.schema !== 'plugin-panel-catalog@1')
            throw new Error('远程目录 schema 不受支持');
        const ids = new Set(parsed.entries.map((entry) => entry.id));
        if (ids.size !== parsed.entries.length || parsed.entries.some((entry) => !entry.id || !entry.title))
            throw new Error('远程目录包含重复或无效条目');
        ...
        if ((parsed.manifest.gaps ?? 0) !== 0)
            throw new Error('远程目录分桶存在缺口，已保留本地旧目录');
     ...
     */
    async fetchPrebuiltCurated(remoteUrl, signal) {
        ...
        if (!Array.isArray(parsed.entries) || parsed.entries.length === 0)
            throw new Error('远程目录文件没有 entries');
        if (parsed.manifest?.schema && parsed.manifest.schema !== 'plugin-panel-curated@1')
            throw new Error('远程目录 schema 不受支持');
        const ids = new Set(parsed.entries.map((entry) => entry.id));
        if (ids.size !== parsed.entries.length || parsed.entries.some((entry) => !entry.id || !entry.title))
            throw new Error('远程目录包含重复或无效条目');
