...
export class CatalogService {
    ...
    async readCache(lens) {
        ...
        try {
            ...
            if (Array.isArray(parsed.entries)) {
                ...
                if (!parsed.entries.some((entry) => entry.installVerified === true))
                    return undefined;
    ...
    async fetchPrebuiltAll(remoteUrl, signal) {
        ...
        if ((parsed.manifest.gaps ?? 0) !== 0 || coveragePct < 99.5)
            throw new Error(`远程目录覆盖率不足（${coveragePct.toFixed(2)}%）`);
        if (previous && parsed.entries.length < previous.entries.length * 0.95)
            throw new Error('远程目录条目异常缩水，已保留本地旧目录');
