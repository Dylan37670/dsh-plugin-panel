...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var zh = {
      ...
      "entry.uninstall": "卸载",
      "entry.cleanup": "清理依赖",
      "entry.inactive": "未启用：缺少 dsh.bundle",
    ...
    var en = {
      ...
      "entry.inactive": "Inactive: missing dsh.bundle",
    ...
    function EntryCard(props) {
      ...
      return el("div", { className: "pp-card", ref: cardRef },
        ...
        el("div", { className: "pp-actions" },
          ...
          installed ? el("span", { className: "pp-installed" }, installed.enabled === false ? (installed.issue || t("entry.inactive")) : t("entry.installed")) : null
        )
      );
    }
    ...
    function PluginPanelApp(props) {
      ...
      function runLifecycle(entry, action) {
        ...
        var body = action === "install" ? { spec: spec, label: entry.title } :
                   action === "update" ? { name: item && item.packageName, spec: item && item.spec } :
                   { name: item && item.packageName };
