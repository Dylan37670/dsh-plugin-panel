...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function PluginPanelApp(props) {
      ...
      function refreshLens(l, full, opLabel) {
        if (fetchingRef.current !== null) return;
        var opId = pushOp(opLabel || (full ? t("src.fetchAll") : t(l === "all" ? "src.fetchingAll" : "src.fetchingCurated")));
        fetchingRef.current = l;
        setFetching(l);
        var timeoutMs = full ? 360000 : 120000;
        api("/refresh", "POST", { remoteCatalogUrl: urlDraft, lens: l, full: !!full }, timeoutMs).then(function (res) {
          if (res.ok) {
            setCatalog({ entries: res.entries, source: res.source, fetchedAt: res.fetchedAt, lens: res.lens, totalHits: res.totalHits });
