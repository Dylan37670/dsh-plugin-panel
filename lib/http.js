...
import { selfStatus, selfUpdate } from "./self-update.js";
        ...
        switch (path) {
            ...
            case '/operations': {
                try {
                    ...
                    sendError(res, 400, 'invalid operation');
                }
                return;
            }
            ...
            case '/install': {
                ...
                if (!spec || !verified) {
                    sendError(res, 400, !verified ? '该条目没有经过验证的安装目标，请打开项目说明查看安装方法。' : 'missing spec');
                    return;
                }
            ...
            case '/update': {
                ...
                const result = await updatePlugin(currentLifecycle(), name, spec, body.manual === true);
                sendJson(res, result.ok ? 200 : 500, result);
                return;
            }
            case '/uninstall': {
                ...
                const result = await uninstallPlugin(currentLifecycle(), name, { manual: body.manual === true, panelManaged: body.panelManaged === true });
                sendJson(res, result.ok ? 200 : 500, result);
                return;
            }
